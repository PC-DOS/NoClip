...
Case 0
Me.txtPassCof.PasswordChar = "*"
txtPassOrg.PasswordChar = "*"
txtPassNew.PasswordChar = "*"
Case 1
txtPassCof.PasswordChar = ""
txtPassOrg.PasswordChar = ""
txtPassNew.PasswordChar = ""
Case Else
txtPassCof.PasswordChar = "*"
...
On Error Resume Next
Dim lpPassOrg As String
Dim lpPassNew As String
Dim lpPassCof As String
lpPassOrg = txtPassOrg.Text
lpPassNew = txtPassNew.Text
lpPassCof = txtPassCof.Text
If PasswordChecksum(lpPassOrg) = False Then
MsgBox "原始密碼輸入錯誤", vbCritical, "Error"
Exit Sub
End If
If lpPassNew <> lpPassCof Then
MsgBox "新密碼與密碼確認不符合", vbCritical, "Error"
Exit Sub
End If
If lpPassNew = "" Then
If UpdatePasswordDatabase(lpPassOrg, "", "") = FUNCTION_SUCCEED Then
MsgBox "已經取消密碼，現在為空密碼狀態", vbInformation, "Info"
Unload Me
Exit Sub
Else
MsgBox "無法完成密碼更新過程，請重試", vbCritical, "Error"
Exit Sub
End If
End If
...
End If
End If
If UpdatePasswordDatabase(lpPassOrg, lpPassNew, lpPassCof) = FUNCTION_SUCCEED Then
MsgBox "密碼已設置", vbInformation, "Info"
Unload Me
Exit Sub
Else
MsgBox "無法完成密碼更新過程，請重試", vbCritical, "Error"
