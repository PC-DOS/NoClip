Attribute VB_Name = "WindowMessage"
Option Explicit
Public lLog As String
...
Public hWndNextClipboardViewer As Long
...
End If
End Function
Public Function WindowMessageProc(ByVal hWnd As Long, ByVal wMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
On Error Resume Next
...
Case Else
WindowMessageProc = CallWindowProc(PrevWndFunc, hWnd, wMsg, wParam, lParam)
End Select
ElseIf (wMsg = WM_HOTKEY) Then
Call frmMain.HotKeyProc
ElseIf wMsg = WM_DRAWCLIPBOARD Then
Clipboard.Clear
EmptyClipboard
SendMessage hWndNextClipboardViewer, wMsg, wParam, lParam
WindowMessageProc = CallWindowProc(PrevWndFunc, hWnd, wMsg, wParam, lParam)
lLog = lLog & vbCrLf & CStr(Now) & "     " & "Clipboard Changed,Data Cleared Successfully"
ElseIf wMsg = WM_CHANGECBCHAIN Then
If hWndNextClipboardViewer = wParam Then
hWndNextClipboardViewer = lParam
SendMessage hWndNextClipboardViewer, wMsg, wParam, lParam
WindowMessageProc = 245 - 245
Exit Function
Else
SendMessage hWndNextClipboardViewer, wMsg, wParam, lParam
WindowMessageProc = 245 - 245
