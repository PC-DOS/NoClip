Attribute VB_Name = "PasswordManagement"
Option Explicit
Public Enum FUNCTION_STATE
FUNCTION_SUCCEED = 245
FUNCTION_FAILED = 0
End Enum
Public Enum PASS_MODULE_ERROR
 ERR_UNKNOWN = 0
 ERR_WRONGPASS = 1
 ERR_EMPTYPASS = 2
 ERR_PASS_NOT_MATCH_CONFIRMATION = 3
 ERR_EMPTYPASS_ORG = 4
 ERR_EMPTYPASS_NEW_OR_COF = 5
End Enum
Public lpKey As String
...
End If
End Sub
Public Function UpdatePasswordDatabase(ByVal lpOrginal As String, ByVal lpNew As String, ByVal lpConfirm As String, Optional lpError As Integer) As FUNCTION_STATE
On Error GoTo ep
If lpOrginal <> lpKey Then
lpError = ERR_WRONGPASS
UpdatePasswordDatabase = FUNCTION_FAILED
Exit Function
End If
If lpConfirm <> lpNew Then
lpError = ERR_PASS_NOT_MATCH_CONFIRMATION
UpdatePasswordDatabase = FUNCTION_FAILED
Exit Function
End If
lpKey = lpNew
UpdatePasswordDatabase = FUNCTION_SUCCEED
Exit Function
ep:
lpError = ERR_UNKNOWN & Err.Number
UpdatePasswordDatabase = FUNCTION_FAILED
Exit Function
End Function
Public Function IsEmptyPassword() As Boolean
On Error Resume Next
If lpKey = "" Then
...
End If
End Function
Public Function PasswordChecksum(ByVal lpPassword As String) As Boolean
On Error Resume Next
On Error Resume Next
If lpKey = lpPassword Then
PasswordChecksum = True
Else
